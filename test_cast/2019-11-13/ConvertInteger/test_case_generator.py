...
class ConvertIntegerTestcaseGenerator():
    def __init__(self, output):
        self.output = output

    def gen_bad_cases(self, length):
        A = random.randint(-pow(2, 31), pow(2, 31) - 1)
        B = random.randint(-pow(2, 31), pow(2, 31) - 1)
        self.output(A, B)
...
class TestCaseGenerator:
    ...
    def __init__(self, file_out='input_file', case_num=20):
        ...
        self.listGen = ConvertIntegerTestcaseGenerator(self.output)
...
def get_res(file_in, file_out):
    ...
    with open(file_out, "w") as fout:
        for i in range(0, len(lines), 2):
            A = int(lines[i])
            B = int(lines[i + 1])
            json.dump(sol.convertInteger(A, B), fout)
            fout.write('\n')
