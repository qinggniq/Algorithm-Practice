...
class PrintBinTestcaseGenerator():
...
class TestCaseGenerator:
    ...
    def __init__(self, file_out='input_file', case_num=20):
        ...
        self.listGen = PrintBinTestcaseGenerator(self.output)
...
def get_res(file_in, file_out):
    ...
    with open(file_out, "w") as fout:
        for i in range(0, len(lines)):
            num = json.loads(lines[i])
            json.dump(sol.printBin(num), fout)
            fout.write('\n')
