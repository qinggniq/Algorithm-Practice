...
class StringToURLTestcaseGenerator():
    ...
    def gen_bad_cases(self, length):
        ...
        while i < length:
            if random.randint(0, 1) % 2 == 0:
                c = AVAILABLE_CHARS[random.randint(
                    0, len(AVAILABLE_CHARS) - 1)]
                ret.append(c)
                if c == ' ':
                    tail += '  '
...
class TestCaseGenerator:
    ...
    def __init__(self, file_out='input_file', case_num=20):
        ...
        self.listGen = StringToURLTestcaseGenerator(self.output)
    ...
    def gen_special_case(self):
        self.output("Mr John Smith    ", 13)
        self.output(' '*15, 5)

        self.output("ds sdfs afs sdfa dfssf asdf             ", 27)
        self.output("      dsfasdfasdfas               ", 19)
...
def get_res(file_in, file_out):
    ...
    with open(file_out, "w") as fout:
        for i in range(0, len(lines), 2):
            ...
            times = json.loads(lines[i + 1])
            fout.write('\"%s\"\n' % sol.replaceSpaces(line, times))
